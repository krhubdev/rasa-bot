...
def write_stories_yaml(pairs: List[tuple], out_path: Path, story_prefix: str = "anxiety"):
    lines = []
    lines.append("version: \"3.1\"")
    lines.append("stories:")
    for idx, (intent, utter) in enumerate(pairs, start=1):
        story_name = f"{story_prefix} {intent} path"
...
def main():
    p = argparse.ArgumentParser(description="Generate Rasa stories.yml from intents and utters.")
    ...
    pr.add_argument("--story-prefix", default="anxiety", help="Prefix for story names")
    ...
    pc.add_argument("--story-prefix", default="anxiety", help="Prefix for story names")

    p.add_argument("--out", required=True, type=Path, help="Output stories.yml")
    ...
    write_stories_yaml(pairs, args.out, story_prefix=args.story_prefix)
